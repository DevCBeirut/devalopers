  ...
  render() {
    ...
    if (searchkey.length > 0) {
      filteredData = data.filter(
        i =>
          i.company.name.toLowerCase().includes(searchkey) ||
    ...
    let renderPageNumbers = pageNumbers.map((number, index) => {
      return (
        <PaginationItem key={index}>
          <PaginationLink onClick={() => this.callPage(number)}>
            {number}
          </PaginationLink>
        </PaginationItem>
      );
            ...
            <div className="row">
              ...
              <div className="jobs-filter col-md-9">
                ...
                <div className="row mb-4">
                  <div className="col-lg-4" style={{ height: 58 }}>
                    <FormControl
                      ...
                      onChange={searchkey => {
                        this.setState({
                          searchkey: searchkey.target.value.toLowerCase(),
                        });
                      }}
                      className="txtsearch mr-sm-2 mt-2 mb-2"
                      style={{
                ...
                {filteredData.map((item, index) => {
                  return (
                    <div key={index} className="jobsitem mb-4">
                      <div className="row">
                        <div className="col-md-7 font-weight-bold">
                          <Link
                            to={'/viewjobinfo/' + item._id}
                            className="text-dark"
                          >
                          ...
                          <h5 className="mb-2 jobdate">
                            {item.company && item.company.name} •{' '}
                            {this.renderDate(item.fromduration)} • (
                            {item.salary ? item.salary : strings.no_salary})
                          </h5>
                        </div>
                        {item.jobtype && (
                          <div className="col-md-5 font-weight-bold text-right mobileleft">
                            {item.acceptremote && (
                              <div className="tag mb-3 btntag  text-secondary ">
                                Remote
                              </div>
                            )}
                            <div className="tag mb-3 btntag  text-secondary ">
